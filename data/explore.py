...
def plot_season_progression(df: pd.DataFrame, season: int):
    ...
    ax.set_title(
        f"{season} Constructors Championship — Points Progression",
        fontsize=15,
        fontweight="bold",
        pad=20,
    )
...
def plot_points_heatmap(df: pd.DataFrame, season: int):
    ...
    pivot = season_df.pivot_table(
        index="constructor_name", columns="round", values="points_this_round", aggfunc="sum"
    ).fillna(0)
    ...
    ax.set_title(
        f"{season} — Points Scored Per Round (by Constructor)",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
...
def plot_championship_gap(df: pd.DataFrame, season: int):
    ...
    ax.set_title(f"{season} — Championship Gap to Leader", fontsize=14, fontweight="bold", pad=15)
    ax.set_xlabel("Race Round", fontsize=12)
...
def print_summary_stats(df: pd.DataFrame):
    ...
    for _, row in champs.iterrows():
        print(
            f"     {int(row['season'])}  →  {row['constructor_name']}  ({int(row['points'])} pts)"
        )
